    ...
    return (
        <Grid display={{xs:'flex',sm:'none'}} flexDirection={'column'}>
            ...
            {dataMobileCardCategory.slice(0,7).map(item => (
                <Grid ml={'1rem'} >
                    <CardProductSwiper bottom={'80%'} left={'-5%'} heightImage={100}widthImage={100} titleBtn={<AddIcon/>} width='130px' title={item.title} image={item.image} Price={item.Price} Weight={item.Weight} priceReduction={item.priceReduction} Discount={item.Discount}id={0}/>
                </Grid>
            ))}
            </Grid>
